...
public:
    Shader(const char *filePath) {
        ...
        try {
            ...
            util::Println("ERROR::SHADER::FILE_NOT_SUCCESFULLY_READ: ",
                          e.what());
        }
        ...
        std::string firstPattern =
            util::Trim(source.substr(index, eol - index));
        ...
        if (firstPattern == "vertex") {
            vertexSource =
                util::Trim(source.substr(piece1from, piece1n)).c_str();
        ...
        if (secondPattern == "vertex") {
            vertexSource =
                util::Trim(source.substr(piece2from, piece2n)).c_str();
